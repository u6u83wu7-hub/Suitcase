...
if ($conn->connect_error) {
    error_log('Order success database connection failed: ' . $conn->connect_error);
...
function successTableColumns($conn, $tableName) {
    $columns = [];
    $safe = preg_replace('/[^a-zA-Z0-9_]/', '', $tableName);
...
$orderItemColumns = $orderItems ? successTableColumns($conn, 'order_items') : [];
                    ...
                    <tbody>
                        <?php foreach ($orderItems as $item): ?>
                            ...
                            if ($hasVariantName && isset($item['variant_name']) && trim((string)$item['variant_name']) !== '') {
                                ...
                                $variantText = trim((string)($item['size_inches'] ?? '') . ((trim((string)($item['size_inches'] ?? '')) !== '' && trim((string)($item['color'] ?? '')) !== '') ? ' / ' : '') . (string)($item['color'] ?? ''));
                            }
                            ?>
