...
// 💡 取得已上傳的 Coupon Banner
$couponBannerMap = [];
$cbResult = $conn->query("SELECT coupon_id, banner_image_url FROM coupon_banners ORDER BY sort_order ASC");
if ($cbResult) {
    while ($row = $cbResult->fetch_assoc()) {
        $cid = (int)$row['coupon_id'];
        if (!isset($couponBannerMap[$cid])) $couponBannerMap[$cid] = [];
        $couponBannerMap[$cid][] = $row['banner_image_url'];
    }
}
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display:flex; gap:6px; flex-wrap:wrap;">
                                        ...
                                        <button class="pm-btn pm-btn-edit pm-btn-sm js-upload-coupon-banner" type="button" data-id="<?php echo intval($coupon['coupon_id']); ?>" data-name="<?php echo h($coupon['coupon_name']); ?>">跑馬燈</button>
...
<div class="coupon-modal" id="couponBannerModal">
    <div class="modal-panel">
        <div class="modal-header">
            <div class="modal-title" id="couponBannerTitle">優惠卷跑馬燈 Banner 設定</div>
            <button type="button" class="pm-btn pm-btn-sub pm-btn-sm" id="closeCouponBannerModal">✕ 關閉</button>
        </div>
        <form action="backend_action.php" method="POST" enctype="multipart/form-data" class="modal-body">
            <?php if (function_exists('apCsrfField')) echo apCsrfField(); ?>
            <input type="hidden" name="action" value="upload_coupon_banner">
            <input type="hidden" name="coupon_id" id="bannerCouponId" value="">
            <div class="pm-grid">
                <div class="pm-col-12">
                    <label for="coupon_banner_image">上傳 Banner 圖片</label>
                    <input class="pm-file-input" type="file" id="coupon_banner_image" name="banner_image" accept="image/*" required>
                    <div style="font-size:12px; color:#64748b; margin-top:6px;">上傳圖片後，此優惠卷將會在首頁的跑馬燈中顯示。建議使用寬版橫幅圖。</div>
                </div>
                <div class="pm-col-6" style="grid-column: span 6;">
                    <label for="coupon_is_show_on_homepage">首頁顯示</label>
                    <select class="pm-select" id="coupon_is_show_on_homepage" name="is_show_on_homepage">
                        <option value="1">是</option>
                        <option value="0">否</option>
                    </select>
                </div>
                <div class="pm-col-6" style="grid-column: span 6;">
                    <label for="coupon_sort_order">排序</label>
                    <input class="pm-input" type="number" id="coupon_sort_order" name="sort_order" value="0">
                </div>
                <div class="pm-col-12">
                    <div class="banner-preview" id="couponBannerPreview"></div>
                </div>
...
<script>
// 💡 JS 變數初始化
const couponBannerMap = <?php echo json_encode($couponBannerMap, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>;
...
// 💡 優惠券跑馬燈 Modal 邏輯
const couponBannerModal = document.getElementById('couponBannerModal');
const closeCouponBannerModal = document.getElementById('closeCouponBannerModal');

document.querySelectorAll('.js-upload-coupon-banner').forEach(btn => {
    btn.addEventListener('click', () => {
        const couponId = btn.dataset.id;
        const couponName = btn.dataset.name || '';
        document.getElementById('bannerCouponId').value = couponId;
        document.getElementById('couponBannerTitle').textContent = '優惠卷跑馬燈 Banner 設定 - ' + couponName;
        
        const preview = document.getElementById('couponBannerPreview');
        preview.innerHTML = '';
        const banners = couponBannerMap[couponId] || [];
        
        if (!document.getElementById('bannerPreviewStyles')) {
            const style = document.createElement('style');
            style.id = 'bannerPreviewStyles';
            style.innerHTML = `
        ...
        if (banners.length === 0) {
            ...
            banners.forEach(url => {
                ...
                form.innerHTML = '<input type="hidden" name="csrf_token" value="' + backendCsrfToken.replace(/"/g, '&quot;') + '">' +
                    '<input type="hidden" name="action" value="delete_coupon_banner">' +
        ...
        couponBannerModal.style.display = 'flex';
    });
});

if (closeCouponBannerModal) closeCouponBannerModal.addEventListener('click', () => { couponBannerModal.style.display = 'none'; });
...
window.addEventListener('click', (event) => {
    ...
    if (event.target === couponBannerModal) couponBannerModal.style.display = 'none';
});
</script>