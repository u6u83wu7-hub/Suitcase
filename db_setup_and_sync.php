...
$sql_coupons = "CREATE TABLE IF NOT EXISTS `coupons` (
    ...
    INDEX `idx_coupons_active_period` (`is_active`, `start_at`, `end_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;";
$conn->query($sql_coupons);

if (!columnExists($conn, 'coupons', 'coupon_type')) {
    $conn->query("ALTER TABLE `coupons` ADD COLUMN `coupon_type` ENUM('DISCOUNT','REDUCE','POINTS') NOT NULL DEFAULT 'DISCOUNT' AFTER `coupon_name`");
}
...
if (columnExists($conn, 'coupons', 'discount_type')) {
    $conn->query("UPDATE `coupons` SET `coupon_type` = CASE `discount_type` WHEN 'PERCENT' THEN 'DISCOUNT' WHEN 'AMOUNT' THEN 'REDUCE' ELSE 'DISCOUNT' END");
}
if (columnExists($conn, 'coupons', 'discount_value')) {
    $conn->query("UPDATE `coupons` SET `coupon_value` = `discount_value` WHERE `coupon_value` = 0.00");
}
if (columnExists($conn, 'coupons', 'coupon_code')) {
    $conn->query("ALTER TABLE `coupons` MODIFY COLUMN `coupon_code` VARCHAR(50) NULL");
}
...
$sql_coupon_distributions = "CREATE TABLE IF NOT EXISTS `coupon_distributions` (
    ...
    `target_type` ENUM('SINGLE','ALL','USE CODE') NOT NULL DEFAULT 'SINGLE',
    `sent_by_admin_id` INT NULL,
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    INDEX `idx_coupon_distribution_coupon` (`coupon_id`),
    INDEX `idx_coupon_distribution_user` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;";
$conn->query($sql_coupon_distributions);

if (columnExists($conn, 'coupon_distributions', 'target_type')) {
    $conn->query("ALTER TABLE `coupon_distributions` MODIFY COLUMN `target_type` ENUM('SINGLE','ALL','USE CODE') NOT NULL DEFAULT 'SINGLE'");
}
...
$sql_coupon_code_uses = "CREATE TABLE IF NOT EXISTS `coupon_code_uses` (
    ...
    UNIQUE KEY `uniq_coupon_code_user` (`coupon_id`, `user_id`),
    INDEX `idx_coupon_code_user` (`user_id`),
    INDEX `idx_coupon_code_value` (`coupon_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;";
...
if (!columnExists($conn, 'orders', 'discount_amount')) {
    $conn->query("ALTER TABLE `orders` ADD COLUMN `discount_amount` DECIMAL(10,2) NOT NULL DEFAULT 0.00 AFTER `coupon_id`");
}
